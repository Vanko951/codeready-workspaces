	...
	node("${node}"){ stage "Build ${DEV_path}, ${PAR_path}, ${CHE_DB_path}, ${CHE_WL_path}, and ${CRW_path}"
		...
		if (env.ghprbPullId && env.ghprbPullId?.trim()) {
			...
			checkout([$class: 'GitSCM', 
				...
				submoduleCfg: [], 
				userRemoteConfigs: [[url: "${CODEREADY_REPO}"]]])
		}
...
:: ${CRW_path} @ ${SHA_CRW} (${VER_CRW})"
		...
		rsync -aPr ../''' + CRW_path + '''/che.d.ts src/components/typings/che.d.ts
		rsync -aPr ../''' + CRW_path + '''/che.d.ts src/components/branding/branding.constant.ts
		# ls -la src/assets/branding/
